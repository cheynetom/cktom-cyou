...
tofu fmt         # format all .tf files (run before committing)
tofu validate    # validate configuration
tofu plan        # preview changes against live Proxmox cluster
tofu apply       # apply changes
```

No test suite. Verification = `tofu plan` output against live cluster.

## Architecture

OpenTofu IaC for the `cktom-cyou` Proxmox VE cluster. Provider: `bpg/proxmox` >= 0.78.0.

### Physical nodes

| Node | CPU | RAM | Storage pools |
|------|-----|-----|---------------|
| node3 | 16 cores | ~62 GB | rpool, datapool0, datapool1 |
| node4 | 8 cores | ~14.6 GB | rpool, datapool0, imagepool0 |

### File layout

- `providers.tf` — provider config (endpoint/creds via variables)
- `variables.tf` — input vars (copy `terraform.tfvars.example` → `terraform.tfvars`)
- `networking.tf` — Linux bridges and SDN zones for both nodes
- `node3.tf` / `node4.tf` — VM/CT definitions per node (11–12 resources each)
- `pools.tf` — Proxmox resource pools
- `imports.tf` — import blocks for existing infra; **remove blocks after successful apply**

### Networking

- Both nodes: VLAN-aware bridges on `vmbr0`; VLANs 3 (mgmt/DHCP) and 4 (secondary/DHCP)
- node3 only: `vmbr1` (10G SFP+, MTU 9000, sniff/mirror), `vmbr2` (2.5GbE, VLANs 2–4)
- SDN internal zones provide NAT'd networks:
  - node3: `10.13.0.0/24` → SNAT via `10.0.3.13`
  - node4: `10.14.0.0/24` → SNAT via `10.0.3.14`
- DNS domain: `<node>.sf.cktom.cyou`; dnsmasq with static leases only

### Key conventions

- Resource naming: `node<X>_<vm_name>` (e.g. `node3_nast`, `node4_pdm`)
- All resources use `lifecycle { ignore_changes = all }` — Tofu defines *baseline* config only; runtime changes via Proxmox UI are not reverted
- VM disks map to ZFS zvols on the respective node's pools

## Workflow (Haft)

Project uses `.haft/` for structured decision tracking:
- Bias toward small, reversible changes
- Require explicit decisions for core/domain edits (networking, storage, VM specs)
- Verify with concrete evidence (`tofu plan` output) before marking work complete
- Use tactical mode only for low-risk docs or narrow test-only fixes
## Communication Style
